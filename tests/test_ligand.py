...
def test_ligand_type(lig_type, lig_type_value):
...
def test_ligand_init(lig_type, lig_str, num, seq_id, actual_num):
...
def test_ligand_to_dict(lig_type, lig_str, seq_id):
