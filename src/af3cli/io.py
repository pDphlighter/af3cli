...
class JSONReader(object):
    """
    Provides static methods for parsing and validating JSON formated AlphaFold3
    input files.

    This class provides functionality to read, validate, and parse a JSON file
    containing all data entries of sequences, ligands, modifications, templates,
    and other related information. It constructs an `InputFile` object encompassing
    the structured data.
    """
    @staticmethod
    ...
    def _parse_ligand(seq_content: dict) -> Ligand:
        ...
        """
        seq_id = JSONReader._get_id(seq_content)
    ...
    def _parse_sequence(seq_type: str, seq_content: dict) -> Sequence:
        ...
        if "modifications" in seq_content.keys():
            modifications = JSONReader._parse_modifcations(seq_type, seq_content)
    ...
    @staticmethod
    def read(filename: str, check: bool = True) -> InputFile:
        """
        ...
        """
        data = JSONReader._read(filename)
        if check:
            JSONReader._check_data(data)
        ...
        for sequence in data["sequences"]:
            ...
            if seq_type == "ligand":
                ligand = JSONReader._parse_ligand(seq_content)
                builder.add_ligand(ligand)
                continue

            next_sequence = JSONReader._parse_sequence(seq_type, seq_content)
